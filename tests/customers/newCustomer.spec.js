...
test('Add new customer', async ({ page }) => {
  ...
  await customer.addNewCustomer(testData.customer.newCustomer);
   
  // await customer.verifyCustomerCreated(
  //   `${testData.customer.newCustomer.firstName} ${testData.customer.newCustomer.lastName}`
  // );
});
